 ...
 * (owned by the course, not by this assignment — see PA4). POST /checkout
 * accepts exactly this shape. The request is validated against the JSON
 * Schema itself at runtime (see server.ts); this file is a plain TypeScript
 * mirror of it for editor/tsc support, not a second source of truth. If the
 * two ever disagree, the schema wins.
 *
 * There is no amount and no recipient in it. The orchestrator derives both:
